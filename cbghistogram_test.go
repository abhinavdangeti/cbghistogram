...
func TestNewHistogram(t *testing.T) {
	tests := []struct {
		...
		{"TestNewHistogram_bins_1", 1},
		{"TestNewHistogram_bins_10", 10},
	}

	for _, test := range tests {
		hist := NewHistogram(test.name, test.bins)
		if hist._name != test.name {
			t.Errorf("Histogram name doesn not match!")
		}

		// Account for 2 additional bins: smallest and largest
		// created by the Histogram
...
func TestParallelAdd(t *testing.T) {
	hist := NewHistogram("TestParallelAdd", 10)
...
func BenchmarkAdd(b *testing.B) {
	hist := NewHistogram("BenchmarkAdd", 100)
...
func BenchmarkGraph(b *testing.B) {
	hist := NewHistogram("BenchMarkGraph", 100)
